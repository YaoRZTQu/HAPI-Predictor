...
from Predict.app.models import ChatHistory, ChatMessage, User
...
async def ask_medical_question(question: Question, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            # 构建消息体 - 使用OpenAI兼容格式
            payload = {
                "model": "deepseek-chat",  # 或使用"deepseek-coder"等其他可用模型
                "messages": [
                    {"role": "system", "content": "你是一个专业的医学顾问，提供准确、详细的医学知识和建议。请记住，你的回答仅供参考，不能替代专业医生的诊断。"},
            ...
            logger.info(f"发送请求到DeepSeek API，URL: {DEEPSEEK_API_URL}")
            
            try:
                async with session.post(DEEPSEEK_API_URL, headers=headers, json=payload) as response:
                    response_text = await response.text()
                    logger.info(f"DeepSeek API响应状态: {response.status}, 响应内容: {response_text[:200]}...")
                    
                    if response.status != 200:
                        logger.error(f"DeepSeek API响应错误: status={response.status}, response={response_text}")
                        return JSONResponse(
                            status_code=500,
                            content={"error": "模型服务器响应错误", "details": f"状态码: {response.status}, 响应: {response_text[:200]}"}
                        )
                    ...
                    if not result or "choices" not in result or not result["choices"]:
                        logger.error(f"DeepSeek API返回无效响应: {result}")
                        return JSONResponse(
                            status_code=500,
                ...
                logger.error(f"连接DeepSeek API失败: {str(e)}")
                return JSONResponse(
                    status_code=500,
        ...
        logger.error(f"处理请求时发生错误: {str(e)}", exc_info=True)  # 添加完整堆栈跟踪
        return JSONResponse(
            status_code=500,
