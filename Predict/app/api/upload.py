...
async def view_file(path: str = Query(...)):
    ...
    try:
        ...
        if not os.path.exists(file_path):
            # 直接尝试使用path作为相对于项目根目录的路径
            alt_file_path = os.path.join(str(BASE_DIR), path)
            if os.path.exists(alt_file_path):
                file_path = alt_file_path
            else:
                logger.warning(f"请求的文件不存在: {file_path}")
                # 返回默认的图片（而不是404错误）
                default_image_path = os.path.join(str(BASE_DIR), "Predict", "app", "static", "img", "image-not-found.png")
