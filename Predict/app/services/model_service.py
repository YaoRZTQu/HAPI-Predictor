...
# 导入原有的模型和工具函数
from Predict.LoadModel.model import Fullmodel

# 为了兼容性，将 Predict.LoadModel 模块设置为可通过 'model' 名称访问
import sys
import Predict.LoadModel.model
sys.modules['model'] = Predict.LoadModel.model
...
class ModelService:
    ...
    def predict(self, model_id: str, input_data: np.ndarray) -> Dict[str, Any]:
        ...
        # 记录输入图像的原始尺寸
        original_height, original_width = input_data.shape
        logger.info(f"原始输入图像尺寸: {original_width}x{original_height}")
        ...
        try:
            ...
            # 确保结果图像尺寸与输入图像一致
            if result_image.shape != input_data.shape:
                logger.warning(f"重建图像形状不匹配: {result_image.shape} vs 输入 {input_data.shape}")
                # 如果尺寸不匹配，调整为输入尺寸
                result_image = resize(result_image, input_data.shape, mode='reflect', anti_aliasing=True)
            
            logger.info(f"最终结果图像形状: {result_image.shape}, 范围: [{result_image.min()}, {result_image.max()}]")
