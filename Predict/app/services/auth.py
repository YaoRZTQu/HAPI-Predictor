...
# 创建包含用户角色的访问令牌
def create_access_token_with_role(user: User, expires_delta: Optional[timedelta] = None):
    """创建包含用户角色的访问令牌"""
    data = {
        "sub": user.username,
        "role": user.role
...
async def get_current_user_optional(
    ...
    """获取当前用户，如果令牌无效或不存在，则返回None而不是抛出异常"""
    logger.info("可选验证当前用户令牌")
    
    if not token:
        logger.info("未找到认证令牌，以匿名用户继续")
        return None
    ...
    try:
        ...
        username: str = payload.get("sub")
        if username is None:
            logger.warning("令牌中未找到用户名，以匿名用户继续")
            return None
        ...
        # 从解码后的负载中获取用户ID和角色
        token_data.sub = username  # 使用username作为sub
        token_data.role = payload.get("role", "user")  # 获取角色，默认为user
            
        logger.info(f"令牌有效，用户名: {username}")
        return token_data
    except JWTError as e:
        logger.warning(f"解码令牌失败: {str(e)}，以匿名用户继续")
        return None
    except Exception as e:
        logger.warning(f"处理令牌时发生错误: {str(e)}，以匿名用户继续")
        return None 