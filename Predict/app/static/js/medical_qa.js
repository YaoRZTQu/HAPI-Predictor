...
async function checkOllamaService() {
    ...
    try {
        // 显示使用DeepSeek API的信息
        addSystemMessage('系统已连接到DeepSeek API，医疗问答服务正常运行中。');
    } catch (error) {
...
window.addEventListener('load', () => {
    ...
    if (aiMessages.length > 0) {
        // 获取最后一条欢迎消息
        const welcomeMessage = aiMessages[aiMessages.length - 1];
        const welcomeText = welcomeMessage.querySelector('p');
        if (welcomeText) {
            welcomeText.textContent = "您好！我是基于DeepSeek技术的AI医疗助手。请问有什么可以帮您？";
        }
        // 更新语音播放按钮的数据
        const playButton = welcomeMessage.querySelector('.play-button');
        if (playButton) {
            playButton.setAttribute('data-text', "您好！我是基于DeepSeek技术的AI医疗助手。请问有什么可以帮您？");
        }
    }
